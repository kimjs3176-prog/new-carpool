  ...
  async function registerListing() {
    ...
    try {
      const newItem = await createListing({
        ...
        direction: fDir, ride_date: fDate, depart_time: fTime,
        ...(showForm === 'driver' ? { seats: fSeats, cost: fCost } : { people: fPeople, budget: fBudget }),
        days: fDays, tags: fTags,
        ...
        <div className="overlay" onClick={() => setShowForm(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="m-title">
              {showForm==='driver' ? '🚗 드라이버 등록' : '🙋 동승자 등록'}
            </div>
            ...
            <div className="frow" style={{marginBottom:14}}>
              <div className="fg" style={{margin:0}}>
                <label>이름</label>
                <input className="fi ro" value={profile.name} readOnly/>
              </div>
              <div className="fg" style={{margin:0}}>
                <label>부서</label>
                <input className="fi ro" value={profile.dept} readOnly/>
              </div>
            </div>
            ...
            <div className="frow" style={{marginBottom:14}}>
              ...
              <div className="fg" style={{margin:0}}>
                <label>운행 날짜</label>
                <input className="fi" type="date" value={fDate} min={today()} onChange={e => setFDate(e.target.value)}/>
              </div>
            </div>
            ...
            <div className="fg">
              ...
              <div className="day-sel">
                {DAYS.map((d,i) => (
                  <button key={d} className={`day-btn${fDays[i]?' on':''}`}
                    onClick={() => setFDays(prev => prev.map((v,j) => j===i ? (v?0:1) : v))}>{d}
                  </button>
                ))}
              </div>
            </div>
      ...
      {detail && (
        <div className="overlay" onClick={() => setDetail(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="dg">
              <div className="dc">
                <div className="dc-lbl">운행 날짜</div>
                <div className="dc-val" style={{fontSize:13}}>{fmtDate(detail.ride_date)}</div>
              </div>
              <div className="dc">
            ...
            <div style={{marginBottom:14}}>
              <div style={{fontSize:12,color:'var(--gray-400)',fontWeight:600,marginBottom:8}}>운행 요일</div>
              <div className="days-row">
