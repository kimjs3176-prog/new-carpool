...
export default function App() {
  ...
  const [fScheduleType, setFScheduleType] = useState<'single'|'repeat'>('single')
  ...
  function openForm(type: 'driver' | 'passenger') {
    ...
    setFScheduleType('single'); setFRepeatEnd('')
  }
        ...
        <div className="overlay" onClick={() => setShowForm(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="sch-type-wrap">
              <button
                className={`sch-type-btn${fScheduleType==='single'?' sch-active':''}`}
                onClick={() => setFScheduleType('single')}
              >
                <span className="sch-type-icon">📅</span>
                <span className="sch-type-label">개별 일정</span>
                <span className="sch-type-desc">특정 날짜 1회</span>
              </button>
              <button
                className={`sch-type-btn${fScheduleType==='repeat'?' sch-active sch-active-r':''}`}
                onClick={() => setFScheduleType('repeat')}
              >
                <span className="sch-type-icon">🔄</span>
                <span className="sch-type-label">반복 일정</span>
                <span className="sch-type-desc">요일 정기 운행</span>
              </button>
            </div>
            ...
            {fScheduleType === 'single' && (
              <div className="sch-panel sch-panel-single">
                <div className="sch-panel-header">
                  <span>📅</span> 운행 날짜 선택
                </div>
                <input
                  className="fi date-input-lg"
                  type="date"
                  value={fDate}
                  min={today()}
                ...
                />
                {fDate && (
                  <div className="date-preview">{fmtDate(fDate)} 하루만 운행해요</div>
                )}
              </div>
            )}
              ...
              <div className="sch-panel sch-panel-repeat">
                ...
                <div className="day-sel-lg">
                  {DAYS.map((d,i) => (
                    <button
                      key={d}
                      className={`day-btn-lg${fDays[i]?' day-on':''}`}
